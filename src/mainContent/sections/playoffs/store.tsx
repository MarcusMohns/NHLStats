const fetchPlayoffsData = async () => {
  // const playoffsUrl = `https://api.nhle.com/stats/rest/nhl/2025/playoff-bracket/now`; <---- try
  const playoffsUrl = `https://corsproxy.io/?url=https://api-web.nhle.com/v1/playoff-bracket/2025`;
  // Run it by https://corsproxy.io/ to bypass CORS

  try {
    const response = await fetch(playoffsUrl);
    const data = await response.json();
    return data;
  } catch (e) {
    console.error("Error fetching playoffs data from API", e);
    throw e;
  }
};

export const fetchPlayoffs = async () => {
  try {
    const playoffs = await fetchPlayoffsData();
    return playoffs;
  } catch (e) {
    console.error("Error fetching leaders data from API", e);
    return new Error("Error fetching data from the server ☹️");
  }
};

export const initialPlayoffsState = {
  bracketLogo:
    "https://assets.nhle.com/logos/playoffs/png/scp-20242025-horizontal-banner-en.png",
  bracketLogoFr:
    "https://assets.nhle.com/logos/playoffs/png/scp-20242025-horizontal-banner-fr.png",
  series: [
    {
      seriesUrl:
        "/schedule/playoff-series/2025/series-a/senators-vs-mapleleafs",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "A",
      playoffRound: 1,
      topSeedRank: 1,
      topSeedRankAbbrev: "D1",
      topSeedWins: 3,
      bottomSeedRank: 4,
      bottomSeedRankAbbrev: "WC1",
      bottomSeedWins: 1,
      topSeedTeam: {
        id: 10,
        abbrev: "TOR",
        name: {
          default: "Toronto Maple Leafs",
          fr: "Maple Leafs de Toronto",
        },
        commonName: {
          default: "Maple Leafs",
        },
        placeNameWithPreposition: {
          default: "Toronto",
          fr: "de Toronto",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/TOR_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/TOR_dark.svg",
      },
      bottomSeedTeam: {
        id: 9,
        abbrev: "OTT",
        name: {
          default: "Ottawa Senators",
          fr: "Sénateurs d'Ottawa",
        },
        commonName: {
          default: "Senators",
          fr: "Sénateurs",
        },
        placeNameWithPreposition: {
          default: "Ottawa",
          fr: "d'Ottawa",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/OTT_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/OTT_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-b/panthers-vs-lightning",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "B",
      playoffRound: 1,
      topSeedRank: 2,
      topSeedRankAbbrev: "D2",
      topSeedWins: 1,
      bottomSeedRank: 3,
      bottomSeedRankAbbrev: "D3",
      bottomSeedWins: 3,
      topSeedTeam: {
        id: 14,
        abbrev: "TBL",
        name: {
          default: "Tampa Bay Lightning",
          fr: "Lightning de Tampa Bay",
        },
        commonName: {
          default: "Lightning",
        },
        placeNameWithPreposition: {
          default: "Tampa Bay",
          fr: "de Tampa Bay",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/TBL_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/TBL_dark.svg",
      },
      bottomSeedTeam: {
        id: 13,
        abbrev: "FLA",
        name: {
          default: "Florida Panthers",
          fr: "Panthers de la Floride",
        },
        commonName: {
          default: "Panthers",
        },
        placeNameWithPreposition: {
          default: "Florida",
          fr: "de la Floride",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/FLA_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/FLA_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-c/canadiens-vs-capitals",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "C",
      playoffRound: 1,
      topSeedRank: 1,
      topSeedRankAbbrev: "D1",
      topSeedWins: 3,
      bottomSeedRank: 4,
      bottomSeedRankAbbrev: "WC2",
      bottomSeedWins: 1,
      topSeedTeam: {
        id: 15,
        abbrev: "WSH",
        name: {
          default: "Washington Capitals",
          fr: "Capitals de Washington",
        },
        commonName: {
          default: "Capitals",
        },
        placeNameWithPreposition: {
          default: "Washington",
          fr: "de Washington",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_light.svg",
        darkLogo:
          "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_dark.svg",
      },
      bottomSeedTeam: {
        id: 8,
        abbrev: "MTL",
        name: {
          default: "Montréal Canadiens",
          fr: "Canadiens de Montréal",
        },
        commonName: {
          default: "Canadiens",
        },
        placeNameWithPreposition: {
          default: "Montréal",
          fr: "de Montréal",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/MTL_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/MTL_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-d/devils-vs-hurricanes",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "D",
      playoffRound: 1,
      topSeedRank: 2,
      topSeedRankAbbrev: "D2",
      topSeedWins: 3,
      bottomSeedRank: 3,
      bottomSeedRankAbbrev: "D3",
      bottomSeedWins: 1,
      topSeedTeam: {
        id: 12,
        abbrev: "CAR",
        name: {
          default: "Carolina Hurricanes",
          fr: "Hurricanes de la Caroline",
        },
        commonName: {
          default: "Hurricanes",
        },
        placeNameWithPreposition: {
          default: "Carolina",
          fr: "de la Caroline",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/CAR_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/CAR_dark.svg",
      },
      bottomSeedTeam: {
        id: 1,
        abbrev: "NJD",
        name: {
          default: "New Jersey Devils",
          fr: "Devils du New Jersey",
        },
        commonName: {
          default: "Devils",
        },
        placeNameWithPreposition: {
          default: "New Jersey",
          fr: "du New Jersey",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/NJD_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/NJD_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-e/blues-vs-jets",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "E",
      playoffRound: 1,
      topSeedRank: 1,
      topSeedRankAbbrev: "D1",
      topSeedWins: 2,
      bottomSeedRank: 4,
      bottomSeedRankAbbrev: "WC2",
      bottomSeedWins: 2,
      topSeedTeam: {
        id: 52,
        abbrev: "WPG",
        name: {
          default: "Winnipeg Jets",
          fr: "Jets de Winnipeg",
        },
        commonName: {
          default: "Jets",
        },
        placeNameWithPreposition: {
          default: "Winnipeg",
          fr: "de Winnipeg",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/WPG_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/WPG_dark.svg",
      },
      bottomSeedTeam: {
        id: 19,
        abbrev: "STL",
        name: {
          default: "St. Louis Blues",
          fr: "Blues de St. Louis",
        },
        commonName: {
          default: "Blues",
        },
        placeNameWithPreposition: {
          default: "St. Louis",
          fr: "de St. Louis",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/STL_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/STL_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-f/avalanche-vs-stars",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "F",
      playoffRound: 1,
      topSeedRank: 2,
      topSeedRankAbbrev: "D2",
      topSeedWins: 3,
      bottomSeedRank: 3,
      bottomSeedRankAbbrev: "D3",
      bottomSeedWins: 2,
      topSeedTeam: {
        id: 25,
        abbrev: "DAL",
        name: {
          default: "Dallas Stars",
          fr: "Stars de Dallas",
        },
        commonName: {
          default: "Stars",
        },
        placeNameWithPreposition: {
          default: "Dallas",
          fr: "de Dallas",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/DAL_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/DAL_dark.svg",
      },
      bottomSeedTeam: {
        id: 21,
        abbrev: "COL",
        name: {
          default: "Colorado Avalanche",
          fr: "Avalanche du Colorado",
        },
        commonName: {
          default: "Avalanche",
        },
        placeNameWithPreposition: {
          default: "Colorado",
          fr: "du Colorado",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/COL_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/COL_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-g/wild-vs-goldenknights",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "G",
      playoffRound: 1,
      topSeedRank: 1,
      topSeedRankAbbrev: "D1",
      topSeedWins: 2,
      bottomSeedRank: 4,
      bottomSeedRankAbbrev: "WC1",
      bottomSeedWins: 2,
      topSeedTeam: {
        id: 54,
        abbrev: "VGK",
        name: {
          default: "Vegas Golden Knights",
          fr: "Golden Knights de Vegas",
        },
        commonName: {
          default: "Golden Knights",
        },
        placeNameWithPreposition: {
          default: "Vegas",
          fr: "de Vegas",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/VGK_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/VGK_dark.svg",
      },
      bottomSeedTeam: {
        id: 30,
        abbrev: "MIN",
        name: {
          default: "Minnesota Wild",
          fr: "Wild du Minnesota",
        },
        commonName: {
          default: "Wild",
        },
        placeNameWithPreposition: {
          default: "Minnesota",
          fr: "du Minnesota",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/MIN_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/MIN_dark.svg",
      },
    },
    {
      seriesUrl: "/schedule/playoff-series/2025/series-h/oilers-vs-kings",
      seriesTitle: "1st Round",
      seriesAbbrev: "R1",
      seriesLetter: "H",
      playoffRound: 1,
      topSeedRank: 2,
      topSeedRankAbbrev: "D2",
      topSeedWins: 2,
      bottomSeedRank: 3,
      bottomSeedRankAbbrev: "D3",
      bottomSeedWins: 2,
      topSeedTeam: {
        id: 26,
        abbrev: "LAK",
        name: {
          default: "Los Angeles Kings",
          fr: "Kings de Los Angeles",
        },
        commonName: {
          default: "Kings",
        },
        placeNameWithPreposition: {
          default: "Los Angeles",
          fr: "de Los Angeles",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/LAK_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/LAK_dark.svg",
      },
      bottomSeedTeam: {
        id: 22,
        abbrev: "EDM",
        name: {
          default: "Edmonton Oilers",
          fr: "Oilers d'Edmonton",
        },
        commonName: {
          default: "Oilers",
        },
        placeNameWithPreposition: {
          default: "Edmonton",
          fr: "d'Edmonton",
        },
        logo: "https://assets.nhle.com/logos/nhl/svg/EDM_light.svg",
        darkLogo: "https://assets.nhle.com/logos/nhl/svg/EDM_dark.svg",
      },
    },
    {
      seriesTitle: "2nd Round",
      seriesAbbrev: "R2",
      seriesLetter: "I",
      playoffRound: 2,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesTitle: "2nd Round",
      seriesAbbrev: "R2",
      seriesLetter: "J",
      playoffRound: 2,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesTitle: "2nd Round",
      seriesAbbrev: "R2",
      seriesLetter: "K",
      playoffRound: 2,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesTitle: "2nd Round",
      seriesAbbrev: "R2",
      seriesLetter: "L",
      playoffRound: 2,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesLogo:
        "https://assets.nhle.com/logos/playoffs/png/ecf-wordmark-en.png",
      seriesLogoFr:
        "https://assets.nhle.com/logos/playoffs/png/ecf-wordmark-fr.png",
      seriesTitle: "Conference Finals",
      seriesAbbrev: "CF",
      seriesLetter: "M",
      playoffRound: 3,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesLogo:
        "https://assets.nhle.com/logos/playoffs/png/wcf-wordmark-en.png",
      seriesLogoFr:
        "https://assets.nhle.com/logos/playoffs/png/wcf-wordmark-fr.png",
      seriesTitle: "Conference Finals",
      seriesAbbrev: "CF",
      seriesLetter: "N",
      playoffRound: 3,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
    {
      seriesLogo:
        "https://assets.nhle.com/logos/playoffs/png/stanley_cup_final_primary.png",
      seriesLogoFr:
        "https://assets.nhle.com/logos/playoffs/png/stanley_cup_final_primary_fr.png",
      seriesTitle: "Stanley Cup Finals",
      seriesAbbrev: "SCF",
      seriesLetter: "O",
      playoffRound: 4,
      topSeedRank: 0,
      topSeedWins: 0,
      bottomSeedRank: 0,
      bottomSeedWins: 0,
    },
  ],
};

export const initialPlayoffsData = {
  seasonId: 20242025,
  currentRound: 2,
  rounds: [
    {
      roundNumber: 1,
      roundLabel: "1st-round",
      roundAbbrev: "R1",
      series: [
        {
          seriesLetter: "A",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-a/senators-vs-mapleleafs",
          bottomSeed: {
            id: 9,
            abbrev: "OTT",
            wins: 2,
            logo: "https://assets.nhle.com/logos/nhl/svg/OTT_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/OTT_dark.svg",
          },
          topSeed: {
            id: 10,
            abbrev: "TOR",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/TOR_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/TOR_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 10,
          losingTeamId: 9,
        },
        {
          seriesLetter: "B",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-b/panthers-vs-lightning",
          bottomSeed: {
            id: 13,
            abbrev: "FLA",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/FLA_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/FLA_dark.svg",
          },
          topSeed: {
            id: 14,
            abbrev: "TBL",
            wins: 1,
            logo: "https://assets.nhle.com/logos/nhl/svg/TBL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/TBL_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 13,
          losingTeamId: 14,
        },
        {
          seriesLetter: "C",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-c/canadiens-vs-capitals",
          bottomSeed: {
            id: 8,
            abbrev: "MTL",
            wins: 1,
            logo: "https://assets.nhle.com/logos/nhl/svg/MTL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/MTL_dark.svg",
          },
          topSeed: {
            id: 15,
            abbrev: "WSH",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_light.svg",
            darkLogo:
              "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 15,
          losingTeamId: 8,
        },
        {
          seriesLetter: "D",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-d/devils-vs-hurricanes",
          bottomSeed: {
            id: 1,
            abbrev: "NJD",
            wins: 1,
            logo: "https://assets.nhle.com/logos/nhl/svg/NJD_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/NJD_dark.svg",
          },
          topSeed: {
            id: 12,
            abbrev: "CAR",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/CAR_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/CAR_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 12,
          losingTeamId: 1,
        },
        {
          seriesLetter: "E",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink: "/schedule/playoff-series/2025/series-e/blues-vs-jets",
          bottomSeed: {
            id: 19,
            abbrev: "STL",
            wins: 3,
            logo: "https://assets.nhle.com/logos/nhl/svg/STL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/STL_dark.svg",
          },
          topSeed: {
            id: 52,
            abbrev: "WPG",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/WPG_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/WPG_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 52,
          losingTeamId: 19,
        },
        {
          seriesLetter: "F",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-f/avalanche-vs-stars",
          bottomSeed: {
            id: 21,
            abbrev: "COL",
            wins: 3,
            logo: "https://assets.nhle.com/logos/nhl/svg/COL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/COL_dark.svg",
          },
          topSeed: {
            id: 25,
            abbrev: "DAL",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/DAL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/DAL_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 25,
          losingTeamId: 21,
        },
        {
          seriesLetter: "G",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-g/wild-vs-goldenknights",
          bottomSeed: {
            id: 30,
            abbrev: "MIN",
            wins: 2,
            logo: "https://assets.nhle.com/logos/nhl/svg/MIN_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/MIN_dark.svg",
          },
          topSeed: {
            id: 54,
            abbrev: "VGK",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/VGK_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/VGK_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 54,
          losingTeamId: 30,
        },
        {
          seriesLetter: "H",
          roundNumber: 1,
          seriesLabel: "1st-round",
          seriesLink: "/schedule/playoff-series/2025/series-h/oilers-vs-kings",
          bottomSeed: {
            id: 22,
            abbrev: "EDM",
            wins: 4,
            logo: "https://assets.nhle.com/logos/nhl/svg/EDM_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/EDM_dark.svg",
          },
          topSeed: {
            id: 26,
            abbrev: "LAK",
            wins: 2,
            logo: "https://assets.nhle.com/logos/nhl/svg/LAK_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/LAK_dark.svg",
          },
          neededToWin: 4,
          winningTeamId: 22,
          losingTeamId: 26,
        },
      ],
    },
    {
      roundNumber: 2,
      roundLabel: "2nd-round",
      roundAbbrev: "R2",
      series: [
        {
          seriesLetter: "I",
          roundNumber: 2,
          seriesLabel: "2nd-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-i/panthers-vs-mapleleafs",
          bottomSeed: {
            id: 13,
            abbrev: "FLA",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/FLA_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/FLA_dark.svg",
          },
          topSeed: {
            id: 10,
            abbrev: "TOR",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/TOR_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/TOR_dark.svg",
          },
          neededToWin: 4,
        },
        {
          seriesLetter: "J",
          roundNumber: 2,
          seriesLabel: "2nd-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-j/hurricanes-vs-capitals",
          bottomSeed: {
            id: 12,
            abbrev: "CAR",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/CAR_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/CAR_dark.svg",
          },
          topSeed: {
            id: 15,
            abbrev: "WSH",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_light.svg",
            darkLogo:
              "https://assets.nhle.com/logos/nhl/svg/WSH_secondary_dark.svg",
          },
          neededToWin: 4,
        },
        {
          seriesLetter: "K",
          roundNumber: 2,
          seriesLabel: "2nd-round",
          seriesLink: "/schedule/playoff-series/2025/series-k/stars-vs-jets",
          bottomSeed: {
            id: 25,
            abbrev: "DAL",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/DAL_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/DAL_dark.svg",
          },
          topSeed: {
            id: 52,
            abbrev: "WPG",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/WPG_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/WPG_dark.svg",
          },
          neededToWin: 4,
        },
        {
          seriesLetter: "L",
          roundNumber: 2,
          seriesLabel: "2nd-round",
          seriesLink:
            "/schedule/playoff-series/2025/series-l/oilers-vs-goldenknights",
          bottomSeed: {
            id: 22,
            abbrev: "EDM",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/EDM_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/EDM_dark.svg",
          },
          topSeed: {
            id: 54,
            abbrev: "VGK",
            wins: 0,
            logo: "https://assets.nhle.com/logos/nhl/svg/VGK_light.svg",
            darkLogo: "https://assets.nhle.com/logos/nhl/svg/VGK_dark.svg",
          },
          neededToWin: 4,
        },
      ],
    },
  ],
};
